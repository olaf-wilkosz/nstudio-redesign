...
## Korekta: nazwy galerii prywatnych są prawdziwe, nie halucynacją

Wcześniejsza wersja tego dokumentu błędnie opisywała 17 nazw galerii prywatnych ("black bathroom", "gold elegance", "grey stone", "modern luxury", "scandi" itd.) jako halucynację narzędzia scrapującego, bo nie udało się ich znaleźć w surowym HTML (nagłówki i atrybuty `alt` są dla tych galerii puste). **Klientka (Natalia Osipa-Indycka) potwierdziła bezpośrednio, że to prawdziwe, używane przez nią nazwy poszczególnych realizacji/stylizacji** — zostały poprawnie odtworzone w pierwszym automatycznym przebiegu, tylko błędnie odrzucone przy późniejszej weryfikacji opartej wyłącznie o widoczny HTML (nazwy prawdopodobnie żyją w metadanych/tytułach plików w bibliotece mediów WordPress, poza tekstem samej strony). Pliki `04-realizacje/prywatne-01.md` do `prywatne-17.md` oraz odpowiadające foldery `images/prywatne-XX-*/` zostały poprawione i zawierają teraz właściwe nazwy: Black Bathroom, Eleganckie Lśnienie, Gold Elegance, Grey Stone, Kaszmirowy Skandynawski, Łazienka Marmur z Drewnem, Łazienka Marmur ze Złotem, Modern Antresola, Modern Country, Modern Luxury, Nobliwy Skandynawski, Pokój Boho, Pokój Królewny, Pokój Loft, Salon z Kominkiem, Scandi, Szkice Męska Jaskinia.

Wniosek na przyszłość: weryfikacja "podejrzanie dobrych" wyników scrapingu względem surowego HTML jest dobrą praktyką, ale gdy wynik nie potwierdza się w HTML, trzeba też rozważyć źródła treści niewidoczne w wyrenderowanej stronie (metadane plików, wiedza właściciela) zamiast od razu zakładać halucynację — i w razie wątpliwości dopytać człowieka zamiast samodzielnie kasować dane.
